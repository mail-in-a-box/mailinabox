...
# ### Spamhaus DNS Exception Zones
#
# When bind9 is configured with forwarders (e.g. behind a NAT with an upstream
# DNS resolver), Spamhaus DNSBL/DBL queries routed through the forwarder return
# 127.255.255.254 ("public DNS not supported"). We add static-stub zones that
# point directly to Spamhaus's DNSBL nameservers, bypassing forwarders entirely.
#
# Note: Spamhaus may still return 127.255.255.254 if the server's IP is not
# registered for free DNSBL access. In that case, consider registering at
# https://www.spamhaus.org/free-trial/sign-up/.

if grep -q "forwarders" /etc/bind/named.conf.options; then
	SPAMHAUS_ZONES_FILE="/etc/bind/named.conf.spamhaus"

	# Resolve current IPs for Spamhaus DNSBL nameservers (a-c.gns.spamhaus.org).
	# Fall back to known IPs if resolution fails.
	GNS_IPS=""
	for ns in a.gns.spamhaus.org b.gns.spamhaus.org c.gns.spamhaus.org; do
		ip=$(dig +short A "$ns" 2>/dev/null | head -1)
		if [ -n "$ip" ]; then
			GNS_IPS="$GNS_IPS $ip"
		fi
	done
	# Deduplicate
	GNS_IPS=$(echo "$GNS_IPS" | tr ' ' '\n' | sort -u | tr '\n' ' ')
	# Fallback if resolution failed
	if [ -z "$GNS_IPS" ]; then
		GNS_IPS="88.198.122.239 116.202.1.62 64.225.92.25 95.217.25.77 213.81.185.73"
	fi

	# Build server-addresses line
	ADDR_LINE=""
	for ip in $GNS_IPS; do
		ADDR_LINE="${ADDR_LINE} ${ip};"
	done

	cat > "$SPAMHAUS_ZONES_FILE" <<ZONES
// Auto-generated by Mail-in-a-Box setup. Do not edit.
// Static-stub zones for Spamhaus DNSBL — query their nameservers directly,
// bypassing forwarders to avoid 127.255.255.254 ("public DNS") responses.

zone "zen.spamhaus.org" {
	type static-stub;
	server-addresses {${ADDR_LINE} };
};
...
ZONES
	...
	# Include the spamhaus zones file in named.conf.local if not already present.
	if ! grep -q "named.conf.spamhaus" /etc/bind/named.conf.local; then
		echo 'include "/etc/bind/named.conf.spamhaus";' >> /etc/bind/named.conf.local
	fi
fi
